...
export function SessionLocationFields({
  ...
  const safeLocations = safeHallLocations;
  const selectedLocation =
    safeLocations.find((location) => location.id === selectedId) ?? null;
  const selectedHallId = selectedLocation ? selectedLocation.id.replace("hall:", "") : "";
      ...
      {safeLocations.length > 0 ? (
        <div className="space-y-2 rounded-md border bg-muted/30 p-4">
          <div className="space-y-1">
            <Label htmlFor="mappedLectureHall">Use mapped lecture hall</Label>
            <p className="text-xs leading-relaxed text-muted-foreground">
              Select a saved hall GPS capture for this session. Use fresh capture only
              when the classroom has not been mapped yet.
            </p>
          </div>
          <select
